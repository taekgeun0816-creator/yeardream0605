...
-- ROWS BETWEEN UNBOUNDED PREECIDING AND UNBAOUNDED FOLLOWING
...
SELECT
    ...
    , amount
    , SUM(amount) OVER (
        ORDER BY paymentDate ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
        )
FROM payments
...
SELECT 
    DISTINCT country
    , FIRST_VALUE(customerName) OVER (PARTITION By country ORDER BY paymentDate DESC)
    AS firstCustomer
